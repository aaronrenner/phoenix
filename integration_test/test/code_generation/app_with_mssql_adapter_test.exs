defmodule Phoenix.Integration.CodeGeneration.AppWithMSSQLAdapterTest do
  ...
  test "newly generated app has no warnings or errors" do
    with_installer_tmp("app_with_mysql_adapter", fn tmp_dir ->
      ...
      assert_file Path.join(app_root_path, "mix.exs"), ":tds"
      assert_file Path.join(app_root_path, "config/dev.exs"), [~r/username: "sa"/, ~r/password: "some!Password"/]
      assert_file Path.join(app_root_path, "config/test.exs"), [~r/username: "sa"/, ~r/password: "some!Password"/]
      assert_file Path.join(app_root_path, "config/prod.secret.exs"), [~r/url: database_url/]
      assert_file Path.join(app_root_path, "lib/custom_path/repo.ex"), "Ecto.Adapters.Tds"
